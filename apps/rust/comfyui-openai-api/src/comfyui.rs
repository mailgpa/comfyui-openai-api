...
fn convert_v04_graph_to_prompt(workflow: &Value) -> Result<Value, String> {
    ...
    for node in nodes {
        ...
        if is_ignored_node(&class_type) {
            debug!("Skipping non-executable node {} ({})", node_id, class_type);
            continue;
        }
...
fn is_ignored_node(class_type: &str) -> bool {
    matches!(class_type, "Note" | "MarkdownNote")
}
...
mod tests {
    ...
    #[test]
    fn skips_note_nodes_when_converting() {
        let workflow = json!({
            "version": 0.4,
            "nodes": [
                {
        ...
        assert!(prompt.get("10").is_none(), "Note node should be skipped");
        assert!(prompt.get("11").is_none(), "MarkdownNote node should be skipped");
        assert!(prompt.get("12").is_some(), "Regular nodes should remain");
    }
}
